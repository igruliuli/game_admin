...
class AccountController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $account->currency = $request->post('curency');
        $account->save();

        $cashdesk = new Cashdesk();
        $cashdesk->account_id = $account->id;
        $cashdesk->save();
    ...
    public function actionDelete()
    {
        ...
        $cashdesk = Cashdesk::findOne(['account_id' => $accountId]);
    ...
    public function actionStatus()
    {
        ...
        if (!isset($user))
            return $this->createResponse(['status' =>'logout']);
        // sample response
        // {
        ...
        $data = json_decode('{
          ...
          "srv25":"ida=1&idh=0&ps=qwerty&trans=1","srv30":"ida=1740&idh=0&ps=qwerty&trans=1&idsrv=30","srv19":"ida=1740&idh=0&ps=qwerty&trans=1","srv234":"ida=1&idh=0&ps=qwerty","srv6":"ida=1&idh=0&ps=qww","srv7":"ida=1740&idh=0&ps=qwert&trans=1","srv8":"ida=1&idh=0&ps=qww","srv9":"ida=1&idh=0&ps=qwert","srv10":"ida=1740&idh=0&ps=qwert&trans=1","srv11":"ida=1740&idh=0&ps=qwerty&trans=1","srv12":"ida=1&idh=0&ps=qww","srv13":"ida=1&idh=0&ps=qww",
          "kenoemon":1,"rulemon":1,"rulon":1,"pokon":1,"kenoon":1,"kenogon":1,"t_bill":1,"tmz":"+5","st":"success",
          "coin6":"","coin7":"","coin10":"",
          "cfpokmin":500,"cfpokmax":100000,"coin234":"","coin8":"","coin9":"","lgn":"t1740k01","pwd":"a2214","limit":"13000",
          "nick":"00131769","type":"0","key":"964983141"
         }');
        ...
        $data['cashdesk'] = Cashdesk::getBalance($user->account->id);
        $data['hall'] = $user->account->id;
        $games = Games::find()->orderBy(['id'=>SORT_ASC])->all();


        //fortuneLive
        $data['cfrulmin'] = $games[0]->min_bet;
        $data['cfrulmax'] = $games[0]->max_bet;
        //fortuneBet
        $data['cfrulemmin'] = $games[1]->min_bet;
        $data['cfrulemmax'] = $games[1]->max_bet;
        //kenoLive
        $data['cfkenomin'] = $games[2]->min_bet;
        $data['cfkenomax'] = $games[2]->max_bet;
        //kenoJx and Bet
        $data['cfkenoemmin'] = $games[4]->min_bet;
        $data['cfkenoemmax'] = $games[4]->max_bet;
        //kenoGold
        $data['cfkenogmin'] = $games[3]->min_bet;
        $data['cfkenogmax'] = $games[3]->max_bet;
        return $this->createResponse($data);
    }
}