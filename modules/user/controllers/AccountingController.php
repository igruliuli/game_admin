...
class AccountingController extends Controller
{
    ...
    public function actionOperations($account_id)
    {
        ...
        $data = BillHistory::getBillReport($start, $end);

        foreach ($data as &$v){
            if ($v['op_type'] == 22)
                $v['amount'] *= -1;

            $v['note'] = "Пополнение баланса администратором счета {$v['who_add']} для счета {$v['account_id']}";
        }
    ...
    public function actionTransfer($account_id)
    {
        ...
        $bill_history->amount = $sum;
        $bill_history->date = (new \DateTime())->format('Y-m-d h:i:s');
        $bill_history->account_id = $account_id;
        $bill_history->who_add = $user->account_id;
        $bill_history->op_type = $op_type;
        $bill_history->save();
