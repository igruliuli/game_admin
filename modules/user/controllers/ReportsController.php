...
class ReportsController extends Controller
{
    ...
    public function actionOverall()
    {
        ...
        foreach ($bets as $bet) {
            $date = new \DateTime($bet['date']);
            $date = $date->format('Y-m-d');
            $billHistoryPlus = BillHistory::find()->where(['=', 'date', $date])->andWhere(['=', 'op_type',  BillHistory::BALANS_PLUS])->one();
            $billHistoryMinus = BillHistory::find()->where(['=', 'date', $date])->andWhere(['=', 'op_type',  BillHistory::BALANS_MINUS])->one();
            $cashPlus = (isset($billHistoryPlus->amount))?$billHistoryPlus->amount:0;
            $cashMinus = (isset($billHistoryMinus->amount))?$billHistoryMinus->amount:0;
            $result[] = [
                'date' => $date,
                'active_accs' => 1,
                'balBeg' => 0,
