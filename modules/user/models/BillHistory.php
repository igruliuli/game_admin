...
class BillHistory extends \yii\db\ActiveRecord
{
    ...
    public static function getBillReport($start, $end){
        $endDate = (new \DateTime($end));
        $query = new Query();
        $data = $query->select([
            "id", "amount", "who_add","account_id",
            new Expression("date as stamp"), "op_type"
            ])
            ->from('bill_history')
            ->where(['between', 'date', $start, $endDate->format('Y-m-d h:i:s')])
